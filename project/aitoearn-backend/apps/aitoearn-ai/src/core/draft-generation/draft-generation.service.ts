...
import { ASSETS_CONFIG, type AssetsConfig, AssetsService, VideoMetadataService } from '@yikart/assets'
import { AccountType, AppException, buildUrl, CreditsConsumptionSource, FileUtil, getErrorMessage, poll, ResponseCode, retry, UserType } from '@yikart/common'
import {
  AiLogChannel,
  AiLogRepository,
  AiLogStatus,
...
export class DraftGenerationService {
  ...
  constructor(
    ...
    @Inject(ASSETS_CONFIG) private readonly assetsConfig: AssetsConfig,
  ) { }

  /**
   * Build an internally accessible URL for ffmpeg/ffprobe operations.
   * Uses the S3 endpoint + bucket name which is accessible within the Docker network,
   * instead of cdnEndpoint which may not be reachable from inside the container.
   */
  private buildInternalUrl(urlOrPath: string): string {
    const relativePath = FileUtil.trimHost(urlOrPath)
    const endpoint = this.assetsConfig.endpoint?.replace(/\/+$/, '') || ''
    const bucket = this.assetsConfig.bucketName || ''
    const internalBase = bucket ? `${endpoint}/${bucket}` : endpoint
    return buildUrl(internalBase, relativePath)
  }
  ...
  async generateContentV2(
    ...
    try {
      ...
      const model = options?.model ?? 'doubao-seedance-1-0-pro-fast-251015'
      ...
      if (draftType === 'draft') {
        if (existing.plan?.videoPrompt) {
          ...
          try {
            ...
            this.logger.warn({ aiLogId, err }, 'V2: Planner failed, using direct prompt fallback')
            plan = {
              title: options?.prompt?.substring(0, 30) || 'AI Video',
              description: options?.prompt || 'Generated by AI',
              topics: ['ai', 'video'],
              videoPrompt: options?.prompt || '',
            }
            await this.aiLogRepository.updateById(aiLogId, {
      ...
      if (existing.videoUrl && existing.coverUrl) {
        ...
        const fullVideoUrl = this.buildInternalUrl(generatedVideoUrl)
        ...
        videoUrl = FileUtil.trimHost(generatedVideoUrl)
  ...
  getDraftGenerationPricing(): DraftGenerationPricingVoInput {
    ...
    const videoModels = config.ai.models.video.generation
      .filter(v => v.channel === AiLogChannel.Volcengine || v.channel === AiLogChannel.Grok)
      ...
      if (draftType === 'image') {
        ...
        if (existingPlan && existingPlan.imagePrompts.length >= targetImageCount) {
          ...
          try {
            ...
            plan = {
              title: options.prompt?.substring(0, 30) || 'AI Image Text',
              description: options.prompt || 'Generated by AI',
              topics: ['ai', 'image'],
  ...
  async generateDraftFromVideoUrl(
    ...
    const fullVideoUrl = this.buildInternalUrl(dto.videoUrl)
...
Return the result as JSON.`
    ...
    const material = await this.materialRepository.create({
      ...
      topics: plan.topics,
      coverUrl,
      mediaList: [{ url: FileUtil.trimHost(dto.videoUrl), type: MediaType.VIDEO, thumbUrl: coverUrl }],
      useCount: 0,
      autoDeleteMedia: false,
      model: modelName,
      generationParams: {
